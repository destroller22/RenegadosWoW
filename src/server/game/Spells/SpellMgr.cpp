...
void SpellMgr::LoadSpellInfoCorrections()
{
    ...
    /// RenegadosWOW FIXES START HEAR


    /// Fix Max target And Implicit target For Undead GRIP Quest Suport
    ApplySpellFix({ 73309 }, [] (SpellInfo* spellInfo)
    {
        const_cast<SpellEffectInfo*>( spellInfo->GetEffect(EFFECT_0) )->TargetA = SpellImplicitTargetInfo();
        const_cast<SpellEffectInfo*>( spellInfo->GetEffect(EFFECT_0) )->RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_15_YARDS); // 100yards instead of 50000?!
        spellInfo->MaxAffectedTargets = 5;
    });


    /// RenegadosWOW FIXES ENDS HEAR
