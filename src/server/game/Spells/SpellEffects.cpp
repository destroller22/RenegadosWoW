...
void Spell::DoCreateItem(uint32 /*i*/, uint32 itemtype, uint8 context /*= 0*/, std::vector<int32> const& bonusListIDs /*= std::vector<int32>()*/, bool vellum)
{
    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER)
        return;
...
void Spell::EffectEnchantItemPerm(SpellEffIndex effIndex)
{
    ...
    if (itemTarget->IsVellum())
    {
        ...
        unitTarget = player;
        // and add a scroll
        DoCreateItem(effIndex, effectInfo->ItemType, 0, bonusListIDs, true);
        itemTarget = NULL;
        m_targets.SetItemTarget(NULL);
    }
    else
