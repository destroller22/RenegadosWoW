...
/*by mayor
http://www.wowhead.com/quest=13815/making-stumps*/
class spell_quest_Ashenvale_Oaks : public SpellScriptLoader
{
public:
    spell_quest_Ashenvale_Oaks() : SpellScriptLoader("spell_quest_Ashenvale_Oaks") { }

    class spell_quest_Ashenvale_OaksSpellScript : public SpellScript
    {
        PrepareSpellScript(spell_quest_Ashenvale_OaksSpellScript);
              
        void HandleOnHit()
        {
            if (Player* _player = GetCaster()->ToPlayer())
            {
                if (Creature* Roble = _player->FindNearestCreature(34167, 25.0f, true))
                {
                    _player->KilledMonsterCredit(34138,_player->GetGUID());
                    _player->DealDamage(Roble, 200000);
                }
        ...
        void Register() override
        {
            AfterHit += SpellHitFn(spell_quest_Ashenvale_OaksSpellScript::HandleOnHit);
    ...
    SpellScript* GetSpellScript() const override
    {
        return new spell_quest_Ashenvale_OaksSpellScript();
    }
};

void AddSC_ashenvale()
{
	new spell_quest_Ashenvale_Oaks();
