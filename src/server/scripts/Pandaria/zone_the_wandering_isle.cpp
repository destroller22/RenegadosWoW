...
public:
    ...
    struct mob_instructor_zhiAI : public ScriptedAI
    {
        ...
        uint32 nextEmote;
        uint32 QuestUpdateTimmer;

        void Reset()
        {
            ownEmoteTimer = 1000;
            QuestUpdateTimmer = 1000;
            nextEmote = 0;
        }
        ...
        {
            events.Update(diff);
            if (QuestUpdateTimmer <= diff)
            {
                std::list<Player*> playerList;
                GetPlayerListInGrid(playerList, me, 15.0f);
                for (auto player : playerList)
                {
                    if (player->GetQuestStatus(29524) == QUEST_STATUS_COMPLETE)
                    {
                        player->CastSpell(player, 100711);//ID - 100711 See Quest Invis 3
                    }
                }
            }
            else
            {
                QuestUpdateTimmer -= diff;
            }
...
public:
    ...
    struct mob_huojin_trainee_AI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                DoMeleeAttackIfReady();
                me->HandleEmoteCommand(27); //EMOTE_STATE_READYUNARMED
...
/// Quest Escusha al maestro Shan shi
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest)
    {
        if (quest->GetQuestId() == 29790) // Digne de passer
            if (Creature* master = player->SummonCreature(56686, creature->GetPositionX(), creature->GetPositionY(), creature->GetPositionZ(), creature->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN, 0, true))
            {
                ...
                player->AddAura(49416, player);// Generic Quest Invisibility Detection 1
            }
...
/*
    Quest 29791 Npc Shang Xi's Hot Air  55649
*/

class npc_bloom : public CreatureScript
{
public:
    npc_bloom() : CreatureScript("npc_bloom") { }

    struct npc_bloomAI : public ScriptedAI
    {
        npc_bloomAI(Creature* creature) : ScriptedAI(creature)
        {
        ...
        void IsSummonedBy(Unit* summoner) override
        {
            ...
            {
                if (Player->GetQuestStatus(29791) == QUEST_STATUS_INCOMPLETE)
                {
                    if (Creature *Aysa = me->FindNearestCreature(56661, 15.0f, true))
                    {
                        Player->KilledMonsterCredit(55939, ObjectGuid::Empty);
                        Player->KilledMonsterCredit(56378, ObjectGuid::Empty);
                        PlayerGUID = summoner->GetGUID();
                        PhasingHandler::RemovePhase(summoner, 1885);
                        summoner->NearTeleportTo(740.592f, 3666.07f, 189.537f, 2.000);
                    }
                }
            }
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_bloomAI(creature);
    }
};

/*NPC  Master Shang Xi 54608*/
class mob_shan_cave_zhi : public CreatureScript
{
public:
    mob_shan_cave_zhi() : CreatureScript("mob_shan_cave_zhi") { }
    CreatureAI* GetAI(Creature* creature) const
    {
        return new mob_shan_cave_zhiAI(creature);
    }

    struct mob_shan_cave_zhiAI : public ScriptedAI
    {
        mob_shan_cave_zhiAI(Creature* creature) : ScriptedAI(creature) {}
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            if (QuestUpdateTimmer <= diff)
            {
                ...
                for (auto player : playerList)
                {
                    if (player->GetQuestStatus(29414) == QUEST_STATUS_COMPLETE)
                    {
                        player->CastSpell(player, 102396);//ID - 100711 See Quest Invis 3
                    }
                }
            }
            else
...
/*Cambio De phase Automatico*/

class PlayerScript_Phasedoor : public PlayerScript
{
public:
    PlayerScript_Phasedoor() : PlayerScript("Phasedoor"), timer(0) {}
    ...
    void OnUpdate(Player* player, uint32 diff)
    {
        ...
        if ( player->GetQuestStatus(29792) == QUEST_STATUS_INCOMPLETE && timer <= diff )
        {
            if ( player->GetAreaId() == 5828 )
            {              
                PhasingHandler::AddPhase(player, 878);
                PhasingHandler::AddPhase(player, 5829);
            }
...
/*NPC  Master Shang Xi 54609*/
class mob_quest_invi : public CreatureScript
{
public:
    mob_quest_invi() : CreatureScript("mob_quest_invi") { }
    CreatureAI* GetAI(Creature* creature) const
    {
        return new mob_quest_inviAI(creature);
    }

    struct mob_quest_inviAI : public ScriptedAI
    {
        mob_quest_inviAI(Creature* creature) : ScriptedAI(creature) {}
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            if ( QuestUpdateTimmer <= diff )
            {
                ...
                for ( auto player : playerList )
                {
                    if ( player->GetQuestStatus(29523) == QUEST_STATUS_COMPLETE ||
                        player->GetQuestStatus(19418) == QUEST_STATUS_COMPLETE || 
                        player->GetQuestStatus(19418) == QUEST_STATUS_REWARDED ||
                        player->GetQuestStatus(29523) == QUEST_STATUS_REWARDED )                    
                        player->CastSpell(player, 102398);//ID - 100711 See Quest Invis 3
...
enum Questenum
{
    BUDDEN_GREATNESS = 29792
};
enum Killcredit
{
    FOREST_DOOR_TWO_CREDIT = 59947,
    FOREST_DOOR_ONE_CREDIT = 59946
};

class AreaTrigger_at_porte_mandori : public AreaTriggerScript
{
public:
    AreaTrigger_at_porte_mandori() : AreaTriggerScript("AreaTrigger_at_porte_mandori")
    {}

    bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/, bool entered) override
    {
        if (player->GetQuestStatus(BUDDEN_GREATNESS) == QUEST_STATUS_INCOMPLETE);
        {
            player->KilledMonsterCredit(FOREST_DOOR_ONE_CREDIT);

            std::list<Player*> playersNear;
            GetPlayerListInGrid(playersNear, player, 5.0f);
        }
        return true;
    }
};

class AreaTrigger_at_porte_pei_wu : public AreaTriggerScript
{
public:
    AreaTrigger_at_porte_pei_wu() : AreaTriggerScript("AreaTrigger_at_porte_pei_wu")
    {}
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/, bool entered) override
    {
        if (player->GetQuestStatus(BUDDEN_GREATNESS) == QUEST_STATUS_INCOMPLETE);
        {
            player->KilledMonsterCredit(FOREST_DOOR_TWO_CREDIT);

            std::list<Player*> playersNear;
            GetPlayerListInGrid(playersNear, player, 6.0f);
        }
        return true;
    }
};
...
{    
    new AreaTrigger_at_porte_mandori();
    new AreaTrigger_at_porte_pei_wu();
    new mob_quest_invi();
    new mob_shan_cave_zhi();
    new npc_bloom();
