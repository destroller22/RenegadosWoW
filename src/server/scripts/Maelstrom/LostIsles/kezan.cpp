    ...
    class spell_kabummbombSpellScript : public SpellScript
    {
        ...
        void HandleActivateGameobject(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCastItem()->GetOwner())
            {
                ...
                {
                    caster->SummonGameObject(GO_DEPOSIT_CHUNK, Deposit->GetPositionX()-1, Deposit->GetPositionY(), Deposit->GetPositionZ(), 0, QuaternionData(), 0);
                    caster->SummonGameObject(GO_DEPOSIT_CHUNK, Deposit->GetPositionX()+1, Deposit->GetPositionY()-1, Deposit->GetPositionZ(), 0, QuaternionData(), 0);
                    caster->SummonGameObject(GO_DEPOSIT_CHUNK, Deposit->GetPositionX()+1, Deposit->GetPositionY()+1, Deposit->GetPositionZ(), 0, QuaternionData(), 0);
                    Deposit->Delete();
                }
            }
...
void AddSC_kezan()
{
    ...
    /*new npc_defiant_troll(); -- Quest Hecha en SAI */
    new npc_bling_bling();
    new npc_bambo();
    ...
    new npc_meteor_gob();
    new npc_gw();
    /*new npc_hot_rod(); -- Vehiculo en SAI */
    new npc_gl_gob();
    new spell_klaxon();
    new spell_radio();
    ...
    /*new npc_original_hot_rod(); -- Vehiculo en SAI*/
    new npc_friend_gob();
