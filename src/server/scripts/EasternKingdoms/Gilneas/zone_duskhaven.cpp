        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    {
                         if (m_mastiff_counter < 10)
                        {
                            if (Player* player = ObjectAccessor::GetPlayer(*me, m_player_GUID))
                            {
                                if (player->GetQuestStatus(QUEST_LEADER_OF_THE_PACK) == QUEST_STATUS_COMPLETE) {
                                    m_events.Reset();
                                    break;
                                }
                            }
                            ...
                            {
                                me->SummonCreature(NPC_MASTIFF, me->GetNearPosition(frand(5.0f, 8.0f), frand(0.0f, 6.28f)), TEMPSUMMON_TIMED_DESPAWN, urand(30000, 60000));
                            }
                        }
                        m_events.ScheduleEvent(EVENT_SEND_MORE_MASTIFF, 500);
                        break;
                    }
                }
            }
