            ...
            if (apply)
            {
                if (Player* player = who->ToPlayer())
                {
                    if (!m_playerGUID)
                    {
                        ...
                        me->GetMotionMaster()->MovePath(3590500, false);
                    }
                }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                ...
                case EVENT_STARTING_RESCUE_PART2:
                {
                    ...
                    me->GetMotionMaster()->MovePath(35905, false);
                    m_events.ScheduleEvent(EVENT_DISMOUNT_PLAYER, 21000);
                    break;
                }
...
//68228
class spell_save_krennan_aranas_credit : public SpellScriptLoader
{
public:
    spell_save_krennan_aranas_credit() : SpellScriptLoader("spell_save_krennan_aranas_credit") { }

    class spell_save_krennan_aranas_credit_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_save_krennan_aranas_credit_SpellScript);
        ...
        {
            OnCheckCast += SpellCheckCastFn(spell_save_krennan_aranas_credit_SpellScript::chequear);
            OnCast += SpellCastFn(spell_save_krennan_aranas_credit_SpellScript::CastSave);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_save_krennan_aranas_credit_SpellScript();
    }
};
...
public:
    ...
    enum eNPC
    {
        EVENT_CROW_GOAWAY = 101,
        EVENT_CROW_DISAPEAR = 102,
    };
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (Player* player = who->ToPlayer())
                if (me->GetDistance(player) < 15.0f) {
                    m_events.RescheduleEvent(EVENT_CROW_GOAWAY, 300);
                }
        }
...
void AddSC_zone_gilneas_city1()
{
    ...
    new spell_save_krennan_aranas_credit();
};
