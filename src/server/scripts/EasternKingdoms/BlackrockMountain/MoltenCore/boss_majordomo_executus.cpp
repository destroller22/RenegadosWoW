...
class boss_majordomo_ragnaros : CreatureScript
{
public:
    boss_majordomo_ragnaros() : CreatureScript("boss_majordomo_ragnaros") { }

    struct boss_majordomo_ragnarosAI : public ScriptedAI
    {
        boss_majordomo_ragnarosAI(Creature* creature) : ScriptedAI(creature)
        {
            //me->setFaction(FACTION_FRIENDLY);
        }
        ...
        void sGossipSelect(Player* player, uint32 menuId, uint32 OptionIndex) override
        {
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return GetMoltenCoreAI<boss_majordomo_ragnarosAI>(creature);
    }
};
